...
pub fn cmd_flash_cdc(args: &[String]) -> Result<(), String> {
    ...
    // Heavily recommend a firmware backup FIRST. ktflash cannot read firmware off the device
    // (the 0x08 read path does not reach flash), so a botched cross-flash can only be recovered
    // with the ORIGINAL manufacturer image + a working reflash. Require --yes.
    if !acked {
        eprintln!("\n\x1b[1;33m╔══════════════════════════════════════════════════════════════════════╗");
        eprintln!("║  ⚠  BACK UP YOUR FIRMWARE FIRST — this ERASES and rewrites flash.     ║");
        eprintln!("║                                                                      ║");
        eprintln!("║  ktflash CANNOT read firmware off the device yet, so there is no      ║");
        eprintln!("║  automatic backup. If this cross-flash goes wrong, the ONLY way back  ║");
        eprintln!("║  is your device's ORIGINAL manufacturer firmware image + a reflash.   ║");
        eprintln!("║  Save that image now (the vendor's updater / your own copy).          ║");
        eprintln!("║                                                                      ║");
        eprintln!("║  A wrong image or a mismatched chip can leave the dongle bootloader-  ║");
        eprintln!("║  only until you reflash a correct image. Proceed at your own risk.    ║");
        eprintln!("╚══════════════════════════════════════════════════════════════════════╝\x1b[0m");
        return Err(
            "refusing to write without acknowledgement — back up your firmware, then re-run with --yes".into(),
        );
    }
    eprintln!("\n⚠  Writing flash. Ensure you have your ORIGINAL firmware image saved for recovery.");
